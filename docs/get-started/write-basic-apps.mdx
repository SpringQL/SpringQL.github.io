...
Stream data are unbound in nature. Some kind of operations such as simple arithmetic conversion and filtering are naturally applied to each of streaming row.
...
You can use **windows** to split unbound rows into bounded amount of rows.
