...
graph TB
    ...
    sink[[sink_temperature_fahrenheit]] -- "Sink writer (TCP client)" -->
...
graph TB
    ...
    sink_all[[sink_avg_all]] -- "Sink writer (TCP client)" -->
    ...
    sink_avg_by_stock[[sink_avg_by_symbol]] -- "Sink writer (TCP client)" -->
...
Columns with `ROWTIME` keyword are **event time**. SpringQL uses event time for time-based windows. [^1]

[^1]: Wallclock-based time (processing time) will be supported in the future.

## What's next?

You learned just a few applications of SpringQL.

We recommend you to learn fundamental concepts of stream processing in [Learn SpringQL](../learn-springql/introduction-why-to-learn-stream-processing)
...
Also, you may find other meaningful examples in [Real-World Examples](../real-world-example/introduction).
