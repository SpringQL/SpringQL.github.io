...
graph TB
    ...
    fsink(["TCP server (nc)<br>(Temperature records in Fahrenheit)"])
```
...
graph TB
    ...
    src[[source_trade]] -- "Pump: avg__by_stock<br>AVG(amount) GROUP BY symbol<br>(2 sec window)" -->
    sink_by_stock[[sink_by_stock]] -- "Sink writer (TCP client)" -->
    fsink_by_stock(["TCP server (nc)<br>(Average stock amount for each stock)"])
```
